...
def evaluate_territory(board):
    ...
    status = {}
    for r in range(1, board.board_rows + 1):
        for c in range(1, board.board_cols + 1):
            p = Point(row=r, col=c)
            if p in status: 
                continue
            stone = board.place(p)
            if stone is not None: 
                status[p] = board.place(p)
            else:
...
def _collect_region(start_pos, board, visited=None):
    ...
    here = board.place(start_pos)
    deltas = [(-1, 0), (1, 0), (0, -1), (0, 1)]
    for delta_r, delta_c in deltas:
        next_p = Point(row=start_pos.row + delta_r, col=start_pos.col + delta_c)
        if not board.in_grid(next_p):
            continue
        neighbor = board.place(next_p)
        if neighbor == here:
